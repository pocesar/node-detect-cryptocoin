...
detectCryptocoin = function (args, list, hashcheck){
  ...
  hashcheck = typeof hashcheck !== 'undefined' ? hashcheck : true;
  ...
  if (!_.isArray(list)) {
    throw new TypeError('List 2nd parameter must be an array');
  }
  ...
  _.forEach(args, function (address){
    ...
    try {
      ...
      if ((start in detectCryptocoin.index)) {
        coin = _.intersection(detectCryptocoin.index[start], list);

        if (hashcheck && !detectCryptocoin.isValid(address, coin, list)) {
...
detectCryptocoin.isValid = function(address, detected, list){
  try {
    return base58.decode(address) ? true : false;
  } catch (e) {
    return false;
  }
};
